import React, { useState, useEffect, createContext, useImperativeHandle, useContext, useRef, useMemo, useCallback, forwardRef } from 'react';
...
function AnimateHeight(props) {
    var isVisible = props.isVisible, _a = props.ease, ease = _a === undefined ? "easeIn" : _a, duration = props.duration, _b = props.variants, variants = _b === undefined ? {
...
var isDroppable = function (dragSource, dropTargetId, treeContext) {
    ...
    if (dragSource === null) {
        ...
        if (dragSourceNode === undefined || dragSourceId_1 === null) {
            return dropTargetId === rootId || !!(dropTargetNode === null || dropTargetNode === undefined ? undefined : dropTargetNode.droppable);
        }
...
var getInnerIndex = function (listItems, monitor) {
    ...
    listItems.forEach(function (el, key) {
        var _a;
        var flag = compareYCoord(el, ((_a = monitor.getClientOffset()) === null || _a === undefined ? undefined : _a.y) || 0);
        if (pos === "") {
            pos = flag;
        }
        else if (pos !== flag) {
...
var getOuterIndex = function (node, nodeEl, monitor) {
    ...
    var parentListItems = parentList === null || parentList === undefined ? undefined : parentList.querySelectorAll(':scope > [role="listitem"]');
    if (!parentListItems) {
        return null;
    }
...
var getDropTarget = function (node, nodeEl, monitor, context) {
    ...
    var hoverPosition = getHoverPosition(nodeEl, ((_a = monitor.getClientOffset()) === null || _a === undefined ? undefined : _a.y) || 0, context);
    if (!list) {
    ...
    var value = __assign(__assign({ extraAcceptTypes: [], listComponent: "ul", listItemComponent: "li", placeholderComponent: "li", sort: true, insertDroppableFirst: true, enableAnimateExpand: false, dropTargetOffset: 0, initialOpen: false }, props), { openIds: openIds, onDrop: function (dragSource, dropTargetId, placeholderIndex) {
            ...
            ? function (dragSourceId, dropTargetId) {
                return canDropCallback(props.tree, {
                    dragSourceId: dragSourceId !== null && dragSourceId !== undefined ? dragSourceId : undefined,
                    dropTargetId: dropTargetId,
...
var useDragControl = function (ref) {
    ...
    useEffect(function () {
        var _a;
        (_a = ref.current) === null || _a === undefined ? undefined : _a.setAttribute("draggable", dragControlContext.isLock ? "false" : "true");
...
var useDragNode = function (item, ref) {
    ...
    useEffect(function () {
        var node = ref.current;
        node === null || node === undefined ? undefined : node.addEventListener("dragstart", handleDragStart);
        node === null || node === undefined ? undefined : node.addEventListener("touchstart", handleTouchStart, {
            passive: true,
        });
        return function () {
            node === null || node === undefined ? undefined : node.removeEventListener("dragstart", handleDragStart);
            node === null || node === undefined ? undefined : node.removeEventListener("touchstart", handleTouchStart);
        };
    }, [ref]);
    var _a = useDrag({
...
var useOpenIdsHelper = function (tree, initialOpen) {
    ...
    useEffect(function () { return setOpenIds(initialOpenIds); }, [initialOpenIds]);
...
var useContainerClassName = function (parentId, isOver) {
    ...
    var className = (classes === null || classes === undefined ? undefined : classes.container) || "";
    if (isOver && (classes === null || classes === undefined ? undefined : classes.dropTarget)) {
        className = "".concat(className, " ").concat(classes.dropTarget);
    }
    if (parentId === rootId && (classes === null || classes === undefined ? undefined : classes.root)) {
        className = "".concat(className, " ").concat(classes.root);
    }
    if (parentId === rootId && (rootProps === null || rootProps === undefined ? undefined : rootProps.className)) {
...
var Node = function (props) {
    ...
    var className = "";
    if (classes === null || classes === undefined ? undefined : classes.listItem) {
        className =
    ...
    if (isDragging && (classes === null || classes === undefined ? undefined : classes.draggingSource)) {
...
var Placeholder = function (props) {
    ...
    return (React.createElement(Component, { className: (classes === null || classes === undefined ? undefined : classes.placeholder) || "" }, placeholderRender(dragSource, { depth: props.depth })));
};
